# SocialKeys.ai — Definitive Super Plan

This document is the definitive merged implementation plan for **SocialKeys.ai**. It keeps GPT-5.4's detailed, implementation-ready structure as the canonical source, while preserving Gemini's best product and platform-operations insights. The plan is intentionally **local-first, mock-first, and zero-external-infra for development**: a new contributor should be able to clone the repo, copy `.env.example`, run migrations, and start the full stack in roughly five minutes without Redis, cloud storage, or live social credentials.

## A. Model Comparison

GPT-5.4 is the stronger implementation backbone: it provides the production-grade monorepo shape, exact commands, mock-first local development model, full Drizzle schema progression, REST API surface, CI/CD pipeline, milestone definitions, and platform-specific API notes with endpoints, auth flow expectations, and registration details. Gemini is stronger on synthesis and operator usability: its platform registration table is the fastest way to coordinate approvals across vendors, and its wireframe descriptions keep the product experience concrete and accessible. GPT-5.4 under-emphasizes quick comparative registration guidance and some UX framing; Gemini under-specifies schema depth, API surface, CI/CD, and hardening. The definitive plan therefore uses GPT-5.4 as the canonical source and explicitly grafts in Gemini's registration matrix, UX framing, and a few product-level implementation cues.

## B. Local Dev Quickstart

The canonical developer path is npm workspaces with SQLite, inline workers, and mock publishing enabled by default. Teams that prefer pnpm may mirror the same scripts, but the documented commands below remain the source of truth because they align with the GPT-5.4 phase plan and keep clone-to-running deterministic.
...
SocialKeys.ai should be built as a local-first TypeScript monorepo with a zero-external-infrastructure development path, production-shaped module boundaries, and explicit room for later scale-out. GPT-5.4 provides the authoritative version pinning, package boundaries, and engineering rules; Gemini contributes a crisp product summary that keeps the stack grounded in user-facing outcomes.

| Layer | Canonical choice | Notes |
| --- | --- | --- |
| Frontend | React 18.3+, Vite 5.x, TailwindCSS 3.4+, React Router, TanStack Query, Zustand | Fast SPA, component reuse, responsive UI, strong dev ergonomics |
| Backend | Node.js 20 LTS, TypeScript 5.6+, Express 5.x | Simple REST API, scheduler host, OAuth callback surface |
| Database | SQLite + `better-sqlite3` + Drizzle ORM 0.36+ | Zero infra locally, Electron-friendly, migration path to Postgres later |
...
Keep a single root `.env` as the canonical development configuration. All apps and packages should read from that root file via shared config helpers so local setup stays predictable. The reference below is the full GPT-5.4 environment contract and should be treated as the baseline `.env.example`.
...
Use Gemini's table as the fastest operator view, then follow the detailed GPT-5.4 setup steps for each platform. Keep all local redirect URIs pointed at the API app, use sandbox/test accounts whenever possible, and plan vendor approvals early because Meta and TikTok can easily become schedule drivers.
...
GPT-5.4 contains the only production-ready, phase-complete Drizzle schema in the source plans, so it is copied here as the canonical database contract. The phase sections below still include SQL/schema evolution notes, but the code in this section should be treated as the authoritative Drizzle ORM reference.
...
The REST API should remain explicit, stable, and mock-friendly. In development, the same surface must operate cleanly against mock adapters, simulated provider responses, and seeded SQLite data.
...
The following 13 phases are inherited from GPT-5.4 because they already include the necessary implementation detail: goal, exact commands, files to create, local development setup, API details, platform permissions, UI/UX considerations, testing, and error-handling guidance. Short “Gemini additions to preserve” notes keep valuable product ideas that were not as explicit in the more detailed plan.
...
#### Gemini additions to preserve
- Keep the local bring-up checklist explicit: install dependencies, copy `.env.example`, initialize the SQLite database, and verify both frontend and backend are reachable before any platform work begins.
- Allow pnpm aliases for teams that prefer it, but keep the canonical workspace scripts npm-based so clone-to-running stays deterministic.
...
#### Gemini additions to preserve
- The initial account-management REST surface should explicitly cover `POST /api/accounts/link/:platform` and `GET /api/accounts/callback/:platform` semantics, even if the final implementation is namespaced under `/api/v1`.
- The Account Manager cards should show connection state at a glance and expose refresh-token / reconnect actions without opening a separate detail page.
...
#### Gemini additions to preserve
- A rich-text editor such as Tiptap is a reasonable enhancement once plain-text + validation is stable; do not let it slow the MVP, but leave the composer state shape compatible with richer editing later.
- The preview panel should support a focused-platform tab so creators can inspect one destination in detail while still seeing aggregate validation across all selected accounts.
...
#### Gemini additions to preserve
- Keep BullMQ/Redis as an optional future path only. For local-first development and the MVP, the SQLite + inline-worker + cron polling approach remains the default because it preserves zero external infrastructure.
...
#### Gemini additions to preserve
- In addition to generate/rewrite flows, include low-friction actions such as “Fix grammar/spelling” and “Rewrite for [platform]” directly in the composer command bar.
...
#### Gemini additions to preserve
- Preserve the “plan a week of content about [topic]” workflow as the simplest onboarding entry point for the planner.
- After AI generation, users should be able to approve, edit, and convert plan items into scheduled posts in a single pass from the planner view.
...
#### Gemini additions to preserve
- Prioritize native notifications and drag/drop file-system affordances first; deeper desktop-only integrations should wait until the shared web/API product is stable.
...
#### Gemini additions to preserve
- Treat loading skeletons, toast notifications, error boundaries, image optimization, and lazy loading as first-class polish requirements rather than optional nice-to-haves.

## J. UI/UX Wireframe Descriptions
...
- Every major screen needs explicit loading, empty, success, warning, and failure states; never leave users guessing whether a publish, upload, or AI generation is running.
- Mock/sandbox badges should always be visible in local development so users know when actions are simulated versus live.

### 1. Account Manager
- Use a dashboard-style layout with platform sections or cards for LinkedIn, Facebook, Instagram, YouTube, and TikTok.
- Unconnected state: greyed card/logo, concise explanation of what the integration enables, and a prominent **Connect account** CTA.
- Connected state: avatar, account name, handle, account-type badge (Page/Profile/Channel/Business), token/scopes status, last sync time, and quick actions for reconnect, refresh token, set default, or disconnect.
- Include bulk filters for active, warning, error, and re-auth-required accounts; this becomes essential once multiple Instagram/Page connections exist.

### 2. Post Composer
- Preferred desktop layout: three columns. Left = editor, AI actions, links, and media tray. Middle = accounts, scheduling, campaign assignment, validation summary. Right = platform previews and warnings.
- Preserve Gemini's focused preview behavior: allow the user to switch the right panel to the currently inspected platform while still showing aggregate validation counters across all targets.
- Include: “What do you want to share?” text area, media drag/drop, live character counts, platform-specific warnings, schedule vs publish-now actions, autosave state, and AI Assistant entry points for generate/rewrite/fix grammar.
- Show upload progress, transcoding progress, preview generation state, unsupported-format errors, oversized-file errors, and missing-alt-text prompts inline.

### 3. Calendar / Campaign Planner
- Combine GPT-5.4's campaign planner with Gemini's simpler calendar mental model: support monthly and weekly calendar views plus a Kanban/status board.
- Scheduled posts should appear as compact cards color-coded by platform or status (draft/scheduled/published/failed), with drag-and-drop rescheduling and click-through editing.
- Side panel should contain the campaign brief, AI strategy notes, target personas, prompt snippets, and approval controls.
- Top actions: generate month plan, plan a week of content, duplicate campaign, export CSV, and bulk-convert approved ideas into scheduled posts.

### 4. Analytics Dashboard
- Header: date-range picker plus workspace/platform/account/campaign filters.
- Top row: KPI cards for total reach, total engagement, success rate, publish latency, and top campaign.
- Main content: performance-over-time chart, platform comparison/donut, and a failure/remediation panel.
- Bottom content: top-performing posts list plus sortable campaign/post tables.
- Empty state should explicitly coach the user (for example: “Publish 5 posts to unlock trends”).

## K. CI/CD Plan

Keep CI minimal but strict in the MVP: every pull request must prove the monorepo installs cleanly, the schema generates, the applications build, and the main user flows still work in smoke tests. Expand packaging and release automation only after the web/API core is stable.
...
## L. Definition of Done / Milestones
